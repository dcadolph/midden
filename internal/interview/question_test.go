...
func TestGenerateAsksAboutEndings(t *testing.T) {
	...
	got := Generate([]weave.Thread{endedThread("arts martial william", "William- Martial arts")},
		nil, nil, DefaultOptions(observed))
	if len(got) != 1 {
...
func TestGenerateSkipsOngoingThreads(t *testing.T) {
	...
	if got := Generate([]weave.Thread{chore}, nil, nil, DefaultOptions(observed)); len(got) != 0 {
...
func TestGenerateSkipsLightThreads(t *testing.T) {
	...
	if got := Generate([]weave.Thread{light}, nil, nil, DefaultOptions(observed)); len(got) != 0 {
...
func TestGenerateNeverRepeatsAnAnsweredQuestion(t *testing.T) {
	...
	first := Generate([]weave.Thread{th}, nil, nil, DefaultOptions(observed))
	if len(first) != 1 {
	...
	if again := Generate([]weave.Thread{th}, nil, answered, DefaultOptions(observed)); len(again) != 0 {
...
func TestQuestionIDIsStableAcrossRuns(t *testing.T) {
	t.Parallel()
	th := endedThread("arts martial william", "William- Martial arts")
	a := Generate([]weave.Thread{th}, nil, nil, DefaultOptions(observed))[0].ID
	b := Generate([]weave.Thread{th}, nil, nil, DefaultOptions(observed))[0].ID
	if a != b {
		t.Errorf("want a stable id so answers keep matching, got %q then %q", a, b)
	}
	other := endedThread("hip hannah hop", "Hannah- hip hop")
	if c := Generate([]weave.Thread{other}, nil, nil, DefaultOptions(observed))[0].ID; c == a {
...
func TestGenerateOrdersHeaviestFirst(t *testing.T) {
	...
	got := Generate([]weave.Thread{lighter, heavy}, nil, nil, DefaultOptions(observed))
	if len(got) != 2 {
...
func TestCrossingQuestionCarriesBothSides(t *testing.T) {
	...
	got := Generate(nil, []weave.Overlap{c}, nil, DefaultOptions(observed))
	if len(got) != 1 {
...
func TestGenerateEmptyRecord(t *testing.T) {
	t.Parallel()
	if got := Generate(nil, nil, nil, DefaultOptions(observed)); len(got) != 0 {
